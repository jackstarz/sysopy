...
pthread_t       *producers;
pthread_t       *consumers;
pthread_cond_t  buf_readable = PTHREAD_COND_INITIALIZER;
pthread_cond_t  buf_writeable = PTHREAD_COND_INITIALIZER;
...
*start_producing(void *args)
{
  ...
  while (1)
  {
    ...
    while (data.lines_count == config.N)
    {
      pthread_cond_wait(&buf_writeable, &buf_mutex);
    }
    ...
    pthread_cond_broadcast(&buf_readable);
    pthread_mutex_unlock(&buf_mutex);
  }
...
*start_consuming(void *args)
{
  ...
  {
    pthread_mutex_lock(&buf_mutex);
    while (data.lines_count == 0)
    {
      pthread_cond_wait(&buf_readable, &buf_mutex);
    }
    ...
    pthread_cond_broadcast(&buf_writeable);
    pthread_mutex_unlock(&buf_mutex);
...
clean_up()
{
  ...
  pthread_cond_destroy(&buf_writeable);
  pthread_cond_destroy(&buf_readable);
}